...
export default function VisionMission() {
  ...
  // Data for the tabs with separate images
  const tabOptions = [
    {
      id: "vision",
      label: "Our Vision",
  ...
  return (
    <section className="relative flex flex-col w-full py-16 px-4 md:px-8 lg:px-16 xl:px-24 bg-gradient-to-br from-gray-50 to-white overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-72 h-72 bg-gradient-to-br from-orange-100 to-teal-100 rounded-full opacity-30 -translate-y-1/2 translate-x-1/2"></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-tr from-teal-100 to-orange-100 rounded-full opacity-20 translate-y-1/2 -translate-x-1/2"></div>
                ...
                className={`group relative px-8 py-4 rounded-2xl text-lg md:text-xl font-semibold transition-all duration-300 transform hover:scale-105 whitespace-nowrap shadow-lg
                  ${
                    activeTab === tab.id
                      ...
                      : "bg-white border-2 border-gray-200 text-gray-700 hover:border-orange-300 hover:shadow-xl"
                  }`}
              >
                {/* Active indicator */}
                {activeTab === tab.id && (
                ...
                {/* Hover effect */}
                <div className="absolute inset-0 bg-gradient-to-r from-orange-400 to-teal-500 rounded-2xl opacity-0 group-hover:opacity-10 transition-opacity duration-300"></div>
              </button>
            ))}
          </div>
